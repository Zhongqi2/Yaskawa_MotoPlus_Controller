...
{
	std::string IP_addr_robot_a = "192.168.1.71";
	udp_send_robot_a = std::make_shared<udp::UDP_Interface>();
	udp_send_robot_a->ClientSetup(IP_addr_robot_a,POS_CMD_PORT_ROBOT_A);

	udp_get_robot_a = std::make_shared<udp::UDP_Interface>();
	udp_get_robot_a->ServerSetup(POS_STATE_PORT_ROBOT_A);

	std::string IP_addr_robot_b = "192.168.1.74";
	udp_send_robot_b = std::make_shared<udp::UDP_Interface>();
	udp_send_robot_b->ClientSetup(IP_addr_robot_b,POS_CMD_PORT_ROBOT_B);
...
bool HardwareInterface::getJointPos(double *joint_pos, const int robot_index)
{
	...
	cout << "joint_pos_fbk:" << joint_pos[0] << " " << joint_pos[1] << " " << joint_pos[2] << " " << joint_pos[3] << " " << joint_pos[4] << " " << joint_pos[5] << endl;
	return true;
}
...
bool HardwareInterface::setJointPosCmd(const double *joint_pos_des, const int robot_index)
{
	...
	// cout << "joint_pos cmd:" << joint_pos[0] << " " << joint_pos[1] << " " << joint_pos[2] << " " << joint_pos[3] << " " << joint_pos[4] << " " << joint_pos[5] << endl;
	if (robot_index == ROBOT_A)
	{
